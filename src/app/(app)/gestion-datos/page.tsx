...
import { Database, AlertTriangle, Loader2, RefreshCw, Calendar, User } from 'lucide-react';
...
export default function GestionDatosPage() {
    ...
    const { loadedMonths, loadedExecutives } = useMemo(() => {
        if (!visits || visits.length === 0) return { loadedMonths: [], loadedExecutives: [] };
        ...
        return { loadedMonths: sortedMonths, loadedExecutives: Array.from(executiveSet).sort() };
    }, [visits]);
